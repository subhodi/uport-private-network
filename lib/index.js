...
class UPortClient {
      ...
      if (img) {
        if (isIPFS.cid(img)) {
          ...
          fs.readFile(img, (err, data) => {
            if (err) { console.error(err); reject(new Error(err)) }
            this.ipfs.files.add(data, (err, result) => {
              if (err) { console.error(err); reject(new Error(err)) }
              console.log('DID uploaded to IPFS: ', result);
  ...
  shareRequestHandler(uri) {
    ...
    // const payload = { ...info, iss: this.mnid, iat: new Date().getTime(), verified, type: 'shareResp', req }
    const payload = { info, iss: this.mnid, iat: new Date().getTime(), verified, type: 'shareReq', req }
    const response = this.signer(payload)
    if (this.network) {
      return this.verifyJWT(params.requestToken).then(() => this.responseHandler(response, token.callback))
    }
  ...
  consume(uri) {
    if (isShareRequest(uri)) return randomClient.shareRequestHandler(uri)
    ...
    if (isAddAttestationRequest(uri)) return randomClient.addAttestationRequestHandler(uri)
